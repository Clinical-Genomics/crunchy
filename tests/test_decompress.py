...
def test_decompress_spring(first_read: Path, second_read: Path, spring_api: MockSpringProcess):
    ...
    # WHEN decompressing
    res = decompress_spring(spring_path=spring_path, first=first_read, second=second_read, spring_api=spring_api)

    # THEN assert that the function returns True
    assert res is True


def test_decompress_spring_dry_run(first_read: Path, second_read: Path, spring_api: MockSpringProcess):
    """Test top decompress compressed file."""
    # GIVEN a compressed file and a outfile
    spring_path = spring_outpath(first_read)

    # WHEN decompressing
    res = decompress_spring(spring_path=spring_path, first=first_read, second=second_read, spring_api=spring_api, dry_run=True)
